...
export function FacturasPage() {
  ...
  const [isNewFacturaModalOpen, setIsNewFacturaModalOpen] = useState(false);
  ...
  const [newFactura, setNewFactura] = useState({
    ...
    fechaEmision: '',
    fechaVencimiento: '',
    estado: 'pendiente' as 'pagada' | 'pendiente' | 'vencida' | 'cancelada'
  });
  ...
  const handleNewFactura = () => {
    const numeroFactura = `FAC-${String(facturas.length + 1).padStart(3, '0')}`;
    const nuevaFactura: Factura = {
      id: String(facturas.length + 1),
      numeroFactura,
      fechaEmision: newFactura.fechaEmision || new Date().toLocaleDateString('es-ES', { day: 'numeric', month: 'short', year: 'numeric' }),
      fechaVencimiento: newFactura.fechaVencimiento,
      cliente: newFactura.cliente,
      numeroReserva: newFactura.numeroReserva,
      concepto: newFactura.concepto,
      subtotal: parseFloat(newFactura.subtotal) || 0,
      impuestos: parseFloat(newFactura.impuestos) || 0,
      total: (parseFloat(newFactura.subtotal) || 0) + (parseFloat(newFactura.impuestos) || 0),
    ...
    setIsNewFacturaModalOpen(false);
  };
      ...
      <div className="facturas-page">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px', width: '100%' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            <h1 style={{ fontSize: '1.125rem', fontWeight: 600, color: '#0f172a', letterSpacing: '-0.1px', margin: 0 }}>Facturas</h1>
          </div>
          <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start', paddingTop: '0px' }}>
            <Button 
              className="bg-slate-700 hover:bg-slate-800 text-white"
              onClick={() => setIsNewFacturaModalOpen(true)}
              style={{ marginTop: 0 }}
            >
        ...
        {/* Modal Nueva Factura */}
        <Dialog open={isNewFacturaModalOpen} onOpenChange={setIsNewFacturaModalOpen}>
          <DialogContent className="sm:max-w-[600px]">
            <DialogHeader>
              <DialogTitle className="text-2xl font-semibold text-slate-900">Nueva Factura</DialogTitle>
            </DialogHeader>
            <div className="space-y-4 py-4">
              <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="relative">
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        setNewFactura({...newFactura, cliente: e.target.value});
                      }}
                ...
                <div>
                  ...
                  <Input
                    id="numeroReserva"
                    value={newFactura.numeroReserva}
                    onChange={(e) => setNewFactura({...newFactura, numeroReserva: e.target.value})}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="subtotal">Subtotal (US$)</Label>
                  <Input
                    id="subtotal"
                    type="number"
                    step="0.01"
                    value={newFactura.subtotal}
                    onChange={(e) => setNewFactura({...newFactura, subtotal: e.target.value})}
                    placeholder="0.00"
                  />
                </div>
                <div>
                  <Label htmlFor="impuestos">Impuestos (US$)</Label>
                  <Input
                    id="impuestos"
                    type="number"
                    step="0.01"
                    value={newFactura.impuestos}
                    onChange={(e) => setNewFactura({...newFactura, impuestos: e.target.value})}
                    placeholder="0.00"
                  />
                </div>
              </div>
              <div>
                ...
                <Select value={newFactura.estado} onValueChange={(value: any) => setNewFactura({...newFactura, estado: value})}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="flex justify-end gap-3 pt-4">
                <Button variant="outline" onClick={() => setIsNewFacturaModalOpen(false)}>
                  Cancelar
                </Button>
                <Button className="bg-slate-700 hover:bg-slate-800 text-white" onClick={handleNewFactura}>
                  Crear Factura
                </Button>
              </div>
            </div>
              ...
              <div className="space-y-6 py-4">
                ...
                <div className="flex items-center justify-between border-b pb-4">
                  <div className="flex items-center gap-3">
                    <svg width="180" height="36" viewBox="0 0 472 93" fill="none" xmlns="http://www.w3.org/2000/svg">
                      ...
                      <defs>
                        <clipPath id="clip0_267_2_factura">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    ...
                    <p className="text-lg font-semibold text-slate-900 mt-1">{selectedFactura.numeroFactura}</p>
                  </div>
                  <div>
                    <Label className="text-slate-500 text-sm">Estado</Label>
                    <div className="mt-1">
                      <Badge className={`${getEstadoBadge(selectedFactura.estado).bg} ${getEstadoBadge(selectedFactura.estado).text} border-0`}>
                        {getEstadoBadge(selectedFactura.estado).label}
                      </Badge>
                    </div>
                  </div>
                  <div>
                    <Label className="text-slate-500 text-sm">Cliente</Label>
                    <p className="text-base text-slate-900 mt-1">{selectedFactura.cliente}</p>
                  </div>
                  <div>
                    <Label className="text-slate-500 text-sm">N° Reserva</Label>
                    <p className="text-base text-slate-900 mt-1">{selectedFactura.numeroReserva}</p>
                  </div>
                  <div>
                    <Label className="text-slate-500 text-sm">Fecha de Emisión</Label>
                    <p className="text-base text-slate-900 mt-1">{selectedFactura.fechaEmision}</p>
                  </div>
                  <div>
                    <Label className="text-slate-500 text-sm">Fecha de Vencimiento</Label>
                    <p className="text-base text-slate-900 mt-1">{selectedFactura.fechaVencimiento}</p>
                  </div>
                </div>
                <div>
                  <Label className="text-slate-500 text-sm">Concepto</Label>
                  <p className="text-base text-slate-900 mt-1">{selectedFactura.concepto}</p>
                </div>
                <div className="border-t pt-4">
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      <span className="text-slate-600">Subtotal:</span>
                      <span className="text-slate-900 font-medium">US${selectedFactura.subtotal.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-600">Impuestos:</span>
                      <span className="text-slate-900 font-medium">US${selectedFactura.impuestos.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between pt-2 border-t">
                      <span className="text-lg font-semibold text-slate-900">Total:</span>
                      <span className="text-lg font-semibold text-slate-900">US${selectedFactura.total.toFixed(2)}</span>
                    </div>
                  </div>
                </div>
                <div className="flex justify-end gap-3 pt-4">
